    ...
    {   
        ...
        static async Task Main(string[] args)
        {
           ...
           Console.WriteLine("Press '1' to exit, '2' To disconnect");
            ...
            if (args[0]== "1")
                {   
                    var single_board_http_client = new SingleBoardHttpClient("http://127.0.0.1:8000"); // Adjust port if neededs
                    ...
                    usbDotNetCanApi.canFrameReceivedDelegate += async (id, data) => 
                    {
                        try
                        {
                            await single_board_http_client.ProcessCanFrame(id, data);
                        }
                        catch (Exception ex)
                        {
                ...
                if (args[0]== "2")
                {
                   // read remote host ip add from a json file saved to disk
                    ...
                    var single_board_http_client = new SingleBoardHttpClient(remoteHost); // Adjust port if needed
